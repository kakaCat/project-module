...
//import com.helijia.pivot.inventory.common.util.*;
...
//import com.helijia.pivot.inventory.param.TimeInventoryListParam;
...
//import com.helijia.pivot.inventory.spi.TimeInventoryLockPolicy.TimeInventoryLockValues;
...
//    @Autowired
//    TimeInventoryMapper inventoryMapper;
//    @Autowired
//    TimeInventoryLockMapper inventoryLockMapper;
//
//    @Override
//    public TimeInventoryViewModel convert(TimeInventoryModel inventory) {
//        Assert.notNull(inventory, "TimeInventoryModel");
//        InternalTimeInventoryModel internal = getInternalTimeInventoryModel(inventory);
//        TimeInventoryViewModel view = new TimeInventoryViewModel();
//        view.setInventory(inventory);
//        view.setUnitMessage(getUnitMessage(inventory.getUnit()));
//        view.setItems(new ArrayList<TimeInventoryItemModel>());
//
//        Calendar cal = Calendar.getInstance();
//        cal.setTime(inventory.getDate());
//        for (int i = 0; i < internal.getPositionLength(); i++) {
//            Date date = cal.getTime();
//            int state = internal.getState(i);
//            String message = getStateMessage(state);
//            view.getItems().add(new TimeInventoryItemModel(i, date, state, message));
//
//            cal.add(Calendar.MINUTE, internal.getUnitMinutes());
//        }
//
//        return view;
//    }
...
//
//    @Override
//    public List<TimeInventoryViewModel> convert(List<TimeInventoryModel> inventories, TimeInventoryLockParam lockParam) {
//
//        return internalConvert(
//                inventories,
//                lockParam,
//                TimeInventoryUtil.getAllowedStates(TimeInventoryModel.STATE_UNUSED),
...
//        TimeInventoryLockValues values = policy.lock(lockParam);
//
...
//        int positionLength = InternalTimeInventoryModel.getPositionLength(inventories.get(0).getUnit());
//
...
//            if (i!=0 && DateUtil.getIntervalDay(temporaryInventories.get(i-1).getDate(), temporaryInventories.get(i).getDate()) != 1) {
//                preAvailableUnits = 0;
//            }
...
//            TimeInventoryViewModel view = temporaryViews.get(i);
//            if (i!=temporaryViews.size()-1 && DateUtil.getIntervalDay(temporaryInventories.get(i).getDate(), temporaryInventories.get(i+1).getDate()) != 1) {
...
//        // 移除加工过程中产生的多余的时间库存对象
//        int previousDays = 0;
//        while (!inventories.get(0).getDate().equals(temporaryViews.get(previousDays).getInventory().getDate())) {
//            previousDays++;
//        }
...
//        Assert.notNull(lockParam.getScopeId(), "lockParam.scopeId");
//        Assert.notNull(lockParam.getLockScope(), "lockParam.lockScope");
//        Assert.notNull(lockParam.getLockScopeId(), "lockParam.lockScopeId");
//        List<TimeInventoryLockModel> inventoryLocks = inventoryLockMapper.getTimeInventoryLocksByLockScope(lockParam.getLockScope(), lockParam.getLockScopeId(), TimeInventoryLockModel.STATE_ON);
//
...
//                throw ErrorCode.buildException(ErrorCode.OP_FAILED, "multi STATE_ON records with the same lockScope");
//            }
//            inventoryLock = lock;
//        }
//
...
//        for (TimeInventoryViewModel view : views) {
//            map.put(view.getInventory().getDate(), view);
//        }
//
//        List<TimeInventoryItemModel> items = PositionsConverter.decodeLockPositions(inventoryLock.getDate(), inventoryLock.getPositions(), inventoryLock.getUnit());
...
//     * @return
//     */
//    private List<TimeInventoryModel> getTemporaryInventories(List<TimeInventoryModel> inventories, TimeInventoryLockValues lockValues) {
//        int positionLength = InternalTimeInventoryModel.getPositionLength(inventories.get(0).getUnit());
...
//        // 后若干天的时间库存
//        TimeInventoryListParam listParam = new TimeInventoryListParam();
//        Date latestDate = inventories.get(inventories.size()-1).getDate();
...
//        int unit = stdInventory.getUnit();
//        Date startDate = stdInventory.getDate();
//        // [startDate, endDate) 前闭后开，因此在需要查询的最后一天再加一天
//        Date endDate = DateUtil.nextDate(views.get(views.size()-1).getInventory().getDate(), 1);
//        int unitMinutes = InternalTimeInventoryModel.getUnitMinutes(unit);
//        List<TimeInventoryLockModel> inventoryLocks = inventoryLockMapper.getTimeInventoryLocksByScopeAndDates(scope, scopeId, TimeInventoryLockModel.STATE_ON, startDate, endDate);
//        Map<Date, TimeInventoryViewModel> map = new HashMap<>();
//        for (TimeInventoryViewModel view : views) {
//            map.put(view.getInventory().getDate(), view);
//        }
...
//            TimeInventoryViewModel view = map.get(DateUtil.toDate(inventoryLock.getDate()));
//            int minutes = DateUtil.getIntervalMinutes(view.getInventory().getDate(), inventoryLock.getDate());
...
//    }
//
//    private void commonValidate(List<TimeInventoryModel> inventories) {
//        // inventories 不能为空
//        if (CollectionUtils.isEmpty(inventories)) {
//            throw ErrorCode.buildException(ErrorCode.PARAM_CANNOT_BE_NULL, "inventories");
//        }
//
//        for (int i=1;i<inventories.size();i++) {
//            // 要求需转换的日期递增(可以不连续)
//            if (DateUtil.getIntervalDay(inventories.get(i-1).getDate(), inventories.get(i).getDate()) <= 0) {
//                LOGGER.warn("inventory Dates must be increasing. inventories : {}", inventories);
//                throw ErrorCode.buildException(ErrorCode.OP_FAILED, "inventory Dates must be increasing");
//            }
//            // 要求 scope 一致
//            if (inventories.get(i-1).getScope() != inventories.get(i).getScope()) {
//                LOGGER.warn("inventory.scope must be same. inventories : {}", inventories);
//                throw ErrorCode.buildException(ErrorCode.OP_FAILED, "inventory.scope must be same");
//            }
//            // 要求 scopeId 一致
//            if (!inventories.get(i-1).getScopeId().equals(inventories.get(i).getScopeId())) {
//                LOGGER.warn("inventory.scopeId must be same. inventories: {}", inventories);
//                throw ErrorCode.buildException(ErrorCode.OP_FAILED, "inventory.scopeId must be same");
//            }
//            // 要求 unit 一致
//            if (inventories.get(i-1).getUnit() != inventories.get(i).getUnit()) {
//                LOGGER.warn("inventory.unit must be same. inventories: {}", inventories);
//                throw ErrorCode.buildException(ErrorCode.OP_FAILED, "inventory.unit must be same");
//            }
//        }
//    }
//
//    private String getStateMessage(int state) {
//        return resource.getString("Inventory.State." + state);
//    }
//
//    private String getUnitMessage(int unit) {
//        return resource.getString("Inventory.Unit." + unit);
//    }
//
