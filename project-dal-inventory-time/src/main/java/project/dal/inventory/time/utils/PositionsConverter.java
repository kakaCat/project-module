...
import static project.dal.inventory.time.utils.DateUtil.getIntervalMinutes;
...
public class PositionsConverter {
    ...
    public static String encodeLockPositions(Date baseDate, List<TimeInventoryItemModel> items, int unit) {
        ...
        int unitMinutes = TimeUnitState.getTimeUnitState(unit).getLength();
        ...
        for (int i=0;i<items.size();i++) {
            ...
            if (items.get(i).getState() == ByteState.STATE_DISABLED.getState()) {
                buf.append("#1");
            }
        }
    ...
    public static List<TimeInventoryItemModel> decodeLockPositions(Date date, String positions, int unit) {
        ...
        for (int i=0;i<positionList.length;i++) {
            if (positionList[i].contains("#")) {
                stateList[i] = ByteState.STATE_DISABLED.getState();
                positionList[i] = positionList[i].split("#")[0];
            } else {
                stateList[i] = ByteState.STATE_UNUSED.getState();
            }
        }
        ...
        int unitMinutes = TimeUnitState.getTimeUnitState(unit).getUnitValue();
        int positionLength = TimeUnitState.getTimeUnitState(unit).getLength();
    ...
    private static String innerEncodePositions(Date baseDate, List<TimeInventoryItemModel> items, int unit) {
        ...
        for (int i=0;i<items.size();i++) {
            ...
            buf.append(items.get(i).getPosition() + DateUtil.getIntervalDay(baseDate, items.get(i).getDate()) * positionLength);
        }
